...
public class JwtController extends ApiController {
    ...
    @PostMapping("jwt")
    public String createJwt(@RequestBody @Valid UserCredentialsEnter userCredentialsEnter, BindingResult bindingResult) {
        if (bindingResult.hasErrors()) {
            throw new ValidationException(BindingResultUtils.getErrorMessage(bindingResult));
        }

        return jwtService.create(userCredentialsEnter.getLogin(), userCredentialsEnter.getPassword());
    }
}
